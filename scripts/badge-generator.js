...
class BadgeGenerator {
  ...
  generateBadge(label, value, color, options = {}) {
    ...
    const labelWidth = this.calculateTextWidth(label, fontSize) + labelPadding * 2;
    const valueWidth = this.calculateTextWidth(value, fontSize) + valuePadding * 2;
    ...
    try {
      ...
      if (fs.existsSync(lcovPath)) {
        ...
        lines.forEach(line => {
          ...
          if (line.startsWith('FNF:')) functionsFound += parseInt(line.slice(4));
  ...
  parseTestResults() {
    try {
      ...
      const testsPassed = process.env.TESTS_PASSED === 'true' || !process.env.TESTS_PASSED;
  ...
  async generateBadges() {
    ...
    if (coverage) {
      ...
      fs.writeFileSync(path.join(this.badgesDir, 'coverage.svg'), coverageBadge);
      ...
      ['lines', 'statements', 'functions', 'branches'].forEach(type => {
        ...
        fs.writeFileSync(path.join(this.badgesDir, `coverage-${type}.svg`), badge);
