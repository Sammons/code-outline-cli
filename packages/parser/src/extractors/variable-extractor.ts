...
export class VariableExtractor implements BaseExtractor {
   ...
   */
  private extractVariableDeclaratorName(node: TreeSitterParser.SyntaxNode, source: string): string | undefined {
   ...
   */
  private extractDeclarationName(node: TreeSitterParser.SyntaxNode, source: string): string | undefined {
    // Show const/let/var
    ...
    NodeUtils.forEachChild(node, (child, index) => {
      if (index === 0) return; // Skip keyword
   ...
   */
  private extractPropertyName(node: TreeSitterParser.SyntaxNode, source: string): string | undefined {
   ...
   */
  private extractPairName(node: TreeSitterParser.SyntaxNode, source: string): string | undefined {
   ...
   */
  private extractCallExpressionName(node: TreeSitterParser.SyntaxNode, source: string): string | undefined {
