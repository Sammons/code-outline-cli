...
export class ImportExportExtractor implements BaseExtractor {
   ...
   */
  private extractImportName(node: TreeSitterParser.SyntaxNode, source: string): string | undefined {
   ...
   */
  private extractImportClauseNames(importClause: TreeSitterParser.SyntaxNode, source: string): string | undefined {
   ...
   */
  private getImportItemName(node: TreeSitterParser.SyntaxNode, source: string): string | undefined {
    switch (node.type) {
      case 'identifier':
      ...
      case 'namespace_import':
        ...
        return identifier ? `* as ${NodeUtils.getNodeText(identifier, source)}` : '*';
   ...
   */
  private extractImportSource(node: TreeSitterParser.SyntaxNode, source: string): string | undefined {
    const stringNode = NodeUtils.findChildByType(node, 'string');
    return stringNode ? NodeUtils.cleanString(NodeUtils.getNodeText(stringNode, source)) : undefined;
  }

  /**
   * Extract name from export statements
   */
  private extractExportName(node: TreeSitterParser.SyntaxNode, source: string): string | undefined {
    ...
    if (exportedDeclaration) {
      ...
      const identifier = NodeUtils.extractIdentifier(exportedDeclaration, source);
      return identifier;
    }

    // Handle default exports
    const defaultKeyword = NodeUtils.findChildByType(node, 'identifier');
    if (defaultKeyword && NodeUtils.getNodeText(defaultKeyword, source) === 'default') {
