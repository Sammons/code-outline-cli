...
describe('TreeUtils', () => {
  describe('findNodesByType', () => {
    ...
    it('should handle empty children array', () => {
      ...
      const results = TreeUtils.findNodesByType(nodeWithEmptyChildren, 'function_declaration');
  ...
  describe('filterNodes', () => {
    it('should filter nodes by predicate', () => {
      const tree = createComplexTree();
      const predicate: NodePredicate = (node) => node.type === 'method_definition';
      const results = TreeUtils.filterNodes(tree, predicate);
      
      expect(results).toHaveLength(2);
      expect(results.every(node => node.type === 'method_definition')).toBe(true);
    });
  ...
  describe('traverseTree', () => {
    ...
    it('should pass correct depth and parent to visitor', () => {
      ...
      expect(calls.find(call => call.depth === 1 && call.parentType === 'program')).toBeDefined();
      expect(calls.find(call => call.depth === 2 && call.parentType === 'class_declaration')).toBeDefined();
    });
  });
  ...
  describe('getAllLeaves', () => {
    ...
    it('should handle complex tree structures', () => {
      ...
      leaves.forEach(leaf => {
        expect(leaf.children === undefined || leaf.children.length === 0).toBe(true);
      });
    });
  });
  ...
  describe('findFirst', () => {
    ...
    it('should return null when no node matches', () => {
      const tree = createSimpleTree();
      const predicate: NodePredicate = (node) => node.type === 'nonexistent_type';
      const result = TreeUtils.findFirst(tree, predicate);
    ...
    it('should pass correct parameters to predicate', () => {
      const tree = createSimpleTree();
      let predicateCall: { node: NodeInfo; depth: number; parent?: NodeInfo } | null = null;
  ...
  describe('mapTree', () => {
    ...
    it('should pass correct depth and parent to mapper', () => {
      ...
      expect(calls.find(call => call.depth === 1 && call.hasParent)).toBeDefined();
      expect(calls.find(call => call.depth === 2 && call.hasParent)).toBeDefined();
    });
  });
  ...
  describe('cloneTree', () => {
    ...
    it('should clone all nested children', () => {
      ...
      expect(clone.children![2].children![0]).not.toBe(tree.children![2].children![0]);
    });
  ...
  describe('Performance', () => {
    it('should perform tree operations efficiently', () => {
      ...
      TreeUtils.filterNodes(complexTree, node => node.type.includes('declaration'));
